...
export function createCODTable() {
  return new Promise((resolve: (value: string) => void, reject) => {
    db.execAsync(
      ...
      .catch((error) => {
        console.error(
          "🚀 ~ file: cod.local.queries.ts:25 ~ createCODTable ~ error:",
          error,
        );
  ...
  return new Promise(
    (
      ...
      db.execAsync(`DROP TABLE IF EXISTS cod;`)
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: cod.local.queries.ts:44 ~ dropCODTable ~ error:",
            error,
          );
          reject(error);
        });
...
export function insertMultipleCOD(codArr: ICODData[]) {
  return new Promise(
    (
      resolve: (value: { message: string; idsInserted: number[] }) => void,
      reject,
    ) => {
      const codMap = new Map<number, ICODData>();
      codArr.forEach((v) => codMap.set(v.codTypeID, v));
      const cods = [...codMap.values()];
      filterDuplicatedCODS(cods)
        .then(({ notExistingIds }) => {
          if (notExistingIds.length > 0) {
            const codsToInsert = cods
              .filter((v) => notExistingIds.find((id) => id === v.codTypeID))
              .map(
                (v) => `
                        (
                            ${`${v.codTypeID}-${v.lang}`},
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: cod.local.queries.ts:45 ~ filterDuplicatedCODS ~ error:",
            error,
          );
          reject(error);
        });
...
export function getAllCODByLang(lang: Language) {
  return new Promise((resolve: (value: ICODData[]) => void, reject) => {
    db.getAllAsync(
      ...
      .catch((error) => {
        console.error(
          "🚀 ~ file: cod.local.queries.ts:104 ~ getAllCOD ~ error:",
          error,
        );
        reject(error);
      });
      ...
      db.getAllAsync(
        ...
        .then((data) => {
          ...
          const setExistingIds = new Set<number>(
            responseData.map((v) => v.codTypeID),
          );
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: cod.local.queries.ts:125 ~ filterDuplicatedCODS ~ error:",
            error,
          );
          reject(error);
        });
