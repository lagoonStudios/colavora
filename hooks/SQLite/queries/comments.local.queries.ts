...
export function createCommentsTable() {
  return new Promise((resolve: (value: string) => void, reject) => {
    db.execAsync(
      ...
      .catch((error) => {
        console.error(
          "🚀 ~ file: comments.local.queries.ts:28 ~ createCommentsTable ~ error:",
          error,
        );
  ...
  return new Promise(
    (
      ...
      db.execAsync(`DROP TABLE IF EXISTS comments;`)
        ...
        .catch((error) => {
          error(
            "🚀 ~ file: comments.local.queries.ts:44 ~ dropCommentsTable ~ error:",
            error,
          );
          reject(error);
        });
 ...
 */
export function insertMultipleComments(
  comments: { shipmentID: number; comment: string; createdDate: string }[],
) {
  return new Promise((resolve, reject) => {
    filterComments(comments)
      .then((filteredComments) => {
        if (filteredComments.length > 0) {
          ...
          Promise.all(promises)
            ...
            .catch((error) => {
              console.error(
                "🚀 comments.local.queries.ts ~ inserMultipleComments ~ line 63 ~ error:",
                error,
              );
              reject(error);
            });
      ...
      .catch((error) => {
        console.error(
          "🚀 comments.local.queries.ts ~ inserMultipleComments ~ line 74 ~ insertMultipleComments ~ error:",
          error,
        );
        reject(error);
      });
 ...
 */
export function getCommentsByShipmentID({
  shipmentID,
}: {
  shipmentID: number;
}) {
  return new Promise(
    (resolve: (value: IOptionalCommentsProps[]) => void, reject) => {
      db.getAllAsync(
        `
            SELECT 
                comment,
...
export function filterComments(
  ...
  return new Promise(
    (
      resolve: (
        value: { shipmentID: number; comment: string; createdDate: string }[],
      ) => void,
      reject,
    ) => {
      ...
      db.getAllAsync(
        ...
        IN 
            (${comments.map(() => "?")});
        `,
        [...shipmentIds, ...comments.map((v) => `${v.comment}`)],
      )
        .then((data) => {
          const existingComments = data as {
            shipmentID: number;
            comment: string;
            createdDate: string;
          }[];
          const notExistingComments = comments
            .filter((v) => {
              const exists = existingComments.find(
                (c) => c.shipmentID === v.shipmentID && c.comment === v.comment,
              );
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ comments.local.queries.ts ~ filterComments ~ line 125 ~ error:",
            error,
          );
          reject(error);
        });
