...
export function createEventsQueueTable() {
  return new Promise((resolve: (value: string) => void, reject) => {
    db.runAsync(
      ...
      .catch((error) => {
        console.error(
          "🚀 ~ file: eventsQueue.loca.queries.ts:8 ~ returnnewPromise ~ error:",
          error,
        );
        reject(error);
      });
...
export function insertEvent({
  ...
  return new Promise(
    (
      resolve: ({ id, message }: { id: number; message: string }) => void,
      reject,
    ) => {
      db.runAsync(
        `
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: eventsQueue.local.queries.ts:57 ~ insertEventQueue ~ error:",
            error,
          );
          reject(error);
        });
...
export function removeEvent(id: number) {
  return new Promise(
    (resolve: ({ message }: { message: string }) => void, reject) => {
      db.runAsync(`DELETE FROM eventsQueue WHERE id = ${id}`)
        ...
        .catch((error) => {
          console.error(
            "🚀 ~ file: eventsQueue.local.queries.ts:71 ~ removeEventQueue ~ error:",
            error,
          );
          reject(error);
        });
...
export function getEventsQueue() {
  return new Promise((resolve: (value: TEventQueueData[]) => void, reject) => {
    ...
    `,
      ...
      .catch((error) => {
        console.error(
          "🚀 ~ file: eventsQueue.local.queries.ts:100 ~ getEventsQueue ~ error:",
          error,
        );
        reject(error);
      });
  ...
  return new Promise((resolve: (value: TEventQueueData) => void, reject) => {
    try {
      const res = db.getFirstSync(
        `SELECT id, eventType, body, shipmentID FROM eventsQueue WHERE id = '${id}'`,
      );
      const data: TEventQueueData = res as TEventQueueData;
      resolve(data);
    } catch (error) {
      console.error(
        "🚀 ~ file: eventsQueue.local.queries.ts:140 ~ getEventsByType ~ error:",
        error,
      );
      reject(error);
    }
...
export function updateShipmentByException({
  isSync,
  shipmentID,
  reasonCode,
}: {
  ...
  return new Promise((resolve, reject) => {
    db.runAsync(
      ...
      .catch((error) => {
        console.error(
          "🚀 ~ file: shipments.local.queries.ts:358 ~ updateShipmentIsSyncState ~ error:",
          error,
        );
        reject(error);
      });
